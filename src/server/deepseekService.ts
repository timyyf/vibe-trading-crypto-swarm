...
export interface RunDeepSeekAgentsParams {
  symbol: string;
  name: string;
  price: number;
  change24h: number;
...
export async function runDeepSeekAgents(params: RunDeepSeekAgentsParams): Promise<AgentReport[]> {
  ...
  const parsed = await callDeepSeekWithRetry(apiKey, prompt, 2);
  const agents = normalizeAgents(parsed?.agents, 'deepseek', params.agentIds);
  ...
  for (const id of params.agentIds) {
    if (!present.has(id)) {
      agents.push(degradedDeepSeekAgent(id));
    }
  }
...
async function callDeepSeekWithRetry(apiKey: string, prompt: string, retries = 2): Promise<any> {
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 402) {
          throw new Error('saldo insuficiente na conta DeepSeek (HTTP 402)');
        }
  ...
  throw new Error('DeepSeek não retornou resposta válida');
}
